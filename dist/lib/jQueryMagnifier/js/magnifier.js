(function($){
	...
	window[fnName] = function(magnifierAttr){
		...
		_this.eqImg = function(){
			...
			var img = new Image(),
				src = "http://a2.vimage1.com/upload/merchandise/pdcvis/611579/2018/1204/77/0d21f01b-b9a1-471f-990f-42ef2045f59c_5t.jpg";
			...
			function imgLoadEnd(){
				...
				if(img.width > img.height){
					...
					styleCss = "max-width:600px; max-height:47px;top:50%;margin-top:"+(-600/2)+"px";
				}else{

					imgWidth = img.width / (img.height / magnifier.width);
					imgHieght = magnifier.width;
					
					styleCss = "max-width:600px; max-height:347px;left:50%;margin-left:"+(-280/2)+"px";
				}

				imgBox.empty().append('<img src="'+src+'" style="'+styleCss+'" />');
				
				//重新计算移动盒子与图片的倍数
				multiple = magnifier.width / boxMoveViewWidth/6;

				//容器加载图片
				_this.view.empty().append('<img src="'+src+'" width="'+imgWidth*multiple+'" height="'+imgHieght*multiple+'" />');
			
				//偏移量
				deviationXl = (magnifier.width - imgWidth) /2 +10;
				deviationXr = magnifier.width - deviationXl - boxMoveViewWidth + 1-10;//这里额外+1的是要计算容器的左边框
		...
		_this.moveFn = function(e){
			...
			positionX = (endX - (magnifier.width-500)/2)*multiple;
			positionY = (endY - (magnifier.height-350)/2)*multiple;
